...
rotate256_big(OUT syndrome_t *out, IN const syndrome_t *in, IN size_t ymm_num) {
    ...
    bike_static_assert(sizeof(*out) > (YMM_SIZE * (R_YMM + (2 * R_YMM_HALF_LOG2))), rotr_big_err);
    ...
    for (uint32_t idx = R_YMM_HALF_LOG2; idx >= 1; idx >>= 1) {
        ...
        for (size_t i = 0; i < (R_YMM + idx); i++) {
            ...
            const __m256i a =
                _mm256_maskload_epi32((const int *)&out->qw[4 * (i + idx)], all_one_mask);
            __m256i b = _mm256_maskload_epi32((const int *)&out->qw[4 * i], all_one_mask);
...
rotate256_small(OUT syndrome_t *out, IN const syndrome_t *in, size_t count) {
    ...
    __m256i zero_mask2 =
        _mm256_setr_epi8(-0x7A, -0x7A, -0x7A, -0x7A, -0x7A, -0x7A, -0x7A, -0x7A, -0x7C, -0x7C, -0x7C, -0x7C,
    ...
    for (int i = R_YMM; i >= 0; i--) {
        ...
        __m256i in256 = _mm256_maskload_epi32((const int *)&in->qw[4 * i], all_one_mask);
        ...
        const __m256i out256 = _mm256_or_si256(_mm256_srli_epi64(in256, count64),
...
void
rotate_right(OUT syndrome_t *out, IN const syndrome_t *in, IN const uint32_t bitscount) {
